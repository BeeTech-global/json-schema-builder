...
describe('Tests based on standard JSON Schema Test Suite', function () {
  ...
  describe('generic keywords (any instance type)', function () {
    ...
    describe('description', function () {
      test('description', 'description matches description', function () {
        var schema = json.description('description');
        _assert2['default'].equal(schema.description(), 'description');
        return schema;
      });
    });

    describe('title', function () {
      test('title', 'title matches title', function () {
        var schema = json.title('title');
        _assert2['default'].equal(schema.title(), 'title');
        return schema;
      });
    });
